...
public class CarView {
    ...
    public static void showCar(int tid) {
        ...
        for (Car c : list){
            System.out.println(c.getId() + "." + (MenuController.getMenuByMenuId(c.getMenuid())).getName() 
                    + " 数量:" + c.getNum());
        }
    ...
    public static void updateCarByTidAndMid(int tid, int mid) {
        ...
        System.out.println(car.getId() + "." + MenuController.getMenuByMenuId(car.getMenuid()).getName() 
                + " 数量:" + car.getNum());
